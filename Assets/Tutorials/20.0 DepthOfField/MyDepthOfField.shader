Shader "Hidden/DepthOfField" //Hidden不会显示在Material选Shader的下拉列表中
{
    Properties
    {
        ...
        Pass // 2 bokehPass
        {
            ...
            half4 FragmentProgram(Interpolators i) : SV_TARGET
            {
             ...
             /*   half3 color = 0;
                ...
                //for (int u = -4; u <= 4; u++) {
                //    for (int v = -4; v <= 4; v++) {
                //        //float2 o = float2(u, v) * _MainTex_TexelSize.xy * 2; //*2 方式达到不用增加采样点的情况下变得更模糊
                //        float2 o = float2(u, v);
                //        if (length(o) <= 4) {
                ...
                //    float2 o = kernel[k];
                //    o *= _MainTex_TexelSize.xy * 4; //使用一半分辨率
                //    o *= _MainTex_TexelSize.xy * _BokehRadius;
                //    color += tex2D(_MainTex, i.uv + o);
                //}
